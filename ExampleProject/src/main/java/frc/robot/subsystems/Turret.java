...
public class Turret  extends SubsystemBase{
    ...
    public Turret() {
        ...
        config.closedLoop.pid(
            TurretConstants.PID_GAINS.kp,
            TurretConstants.PID_GAINS.ki,
            TurretConstants.PID_GAINS.kd);
        ...
        feedforward = new SimpleMotorFeedforward(TurretConstants.FEEDFORWARD_GAINS.ks, TurretConstants.FEEDFORWARD_GAINS.kv);
        ...
        profile = new TrapezoidProfile(new Constraints(TurretConstants.MAX_VELOCITY.in(RadiansPerSecond), TurretConstants.MAX_ACCELERATION.in(RadiansPerSecondPerSecond)));

        // So we have our path. However, we need to specify a destination. That's what goal state is, its where we want to end up.
        goalState = new State(0, 0);

        // Think of motor setpoint as the steps along that path. It gets updated constantly by the feedforward
        motorSetpoint = new State(0, 0);

        setpoint = Radian.mutable(0);
        position = Radians.mutable(0);

        velocity = RadiansPerSecond.mutable(0);
    ...
    public void periodic() {
        ...
        motorSetpoint = profile.calculate(RobotConstants.LOOP_TIME.in(Seconds), motorSetpoint, goalState);
        ...
        controller.setReference(motorSetpoint.position, ControlType.kPosition, ClosedLoopSlot.kSlot0, feedforward.calculate(motorSetpoint.velocity));

        // Logged stuff
        position.mut_setMagnitude(encoder.getPosition());
    ...
    public void simulationPeriodic() {
        position.mut_replace(motorSetpoint.position, Radians);
        velocity.mut_replace(motorSetpoint.velocity, RadiansPerSecond);
    }
